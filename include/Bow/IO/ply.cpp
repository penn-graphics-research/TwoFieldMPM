...
BOW_INLINE void writeTwoField_particles_ply(const std::string filename, const Field<Vector<T, dim>>& _vertices, const Field<Vector<T, dim>>& velocities, const Field<Vector<T, dim>>& damageGradients, const std::vector<T>& masses, const std::vector<T>& damage, const std::vector<int>& _sp, const Field<int>& _markers, const Field<Matrix<T,dim,dim>>& _m_cauchySmooth, const Field<Matrix<T,dim,dim>>& _m_FSmooth, const Field<T>& m_lamMax, const bool smoothParticleStressField, const Field<Matrix<T,dim,dim>>& _m_cauchy, const Field<Matrix<T,dim,dim>>& _m_F, const Field<T>& m_chemPot, const Field<T>& m_J, const Field<T>& m_pressure, const bool binary)
{
    ...
    Field<T> smooth_sigma11(_vertices.size());
    Field<T> smooth_sigma22(_vertices.size());
    Field<T> smooth_sigma12(_vertices.size());
    Field<T> smooth_F11(_vertices.size());
    ...
    tbb::parallel_for((size_t)0, _vertices.size(), [&](size_t i) {
        ...
        if(_markers[i] == 0 || _markers[i] == 5){ //only write the rest if these are actual particles (rather than crack particles)
            ...
            J[i] = m_J[i];
            pressure[i] = m_pressure[i];

            smoothJ[i] = 0.0;
            smoothPressure[i] = 0.0;
        }
    });
    ...
    if (std::is_same<T, float>::value) {
        ...
        file.add_properties_to_element("vertex", { "smoothSigma11" }, tinyply::Type::FLOAT32, vertices.size(), reinterpret_cast<const uint8_t*>(smooth_sigma11.data()), tinyply::Type::INVALID, 0);
        file.add_properties_to_element("vertex", { "smoothSigma22" }, tinyply::Type::FLOAT32, vertices.size(), reinterpret_cast<const uint8_t*>(smooth_sigma22.data()), tinyply::Type::INVALID, 0);
        file.add_properties_to_element("vertex", { "smoothSigma12" }, tinyply::Type::FLOAT32, vertices.size(), reinterpret_cast<const uint8_t*>(smooth_sigma12.data()), tinyply::Type::INVALID, 0);
        file.add_properties_to_element("vertex", { "smoothF11" }, tinyply::Type::FLOAT32, vertices.size(), reinterpret_cast<const uint8_t*>(smooth_F11.data()), tinyply::Type::INVALID, 0);
        ...
        file.add_properties_to_element("vertex", { "smoothSigma11" }, tinyply::Type::FLOAT64, vertices.size(), reinterpret_cast<const uint8_t*>(smooth_sigma11.data()), tinyply::Type::INVALID, 0);
        file.add_properties_to_element("vertex", { "smoothSigma22" }, tinyply::Type::FLOAT64, vertices.size(), reinterpret_cast<const uint8_t*>(smooth_sigma22.data()), tinyply::Type::INVALID, 0);
        file.add_properties_to_element("vertex", { "smoothSigma12" }, tinyply::Type::FLOAT64, vertices.size(), reinterpret_cast<const uint8_t*>(smooth_sigma12.data()), tinyply::Type::INVALID, 0);
        file.add_properties_to_element("vertex", { "smoothF11" }, tinyply::Type::FLOAT64, vertices.size(), reinterpret_cast<const uint8_t*>(smooth_F11.data()), tinyply::Type::INVALID, 0);
