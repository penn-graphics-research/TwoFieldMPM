...
public:
    ...
    Vector<T, dim> old_v;
    Vector<T, dim> a;
    typename std::conditional<std::is_same<T, float>::value, int32_t, int64_t>::type stencil_idx;
    Vector<T, 16 - 4 * dim - 3> padding;
    void reset()
    {
...
public:
    ...
    template <int interpolation_degree>
    ...
    template <typename OP, int degree>
