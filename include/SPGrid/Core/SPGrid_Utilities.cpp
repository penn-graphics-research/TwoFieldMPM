...
void Deactivate_Page(void* data, const size_t size)
{
    ...
    if (ret < 0) {
        char buffer[256];
#if defined __USE_XOPEN2K && !defined __USE_GNU || __APPLE__
        int error_message = strerror_r(errno, buffer, 256);
        FATAL_ERROR("Failed to deallocate " + Value_To_String(size) + " bytes, ERROR: " + std::to_string(error_message));
#else
        char* error_message = strerror_r(errno, buffer, 256);
        FATAL_ERROR("Failed to deallocate " + Value_To_String(size) + " bytes, ERROR: " + std::string(error_message));
#endif
    }
    #endif
}
