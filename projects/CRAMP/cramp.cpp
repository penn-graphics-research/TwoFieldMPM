...
int main(int argc, char *argv[]) {
  ...
  if (testcase == 0) {
    ...
    return 0;
    // Without Padding
    // NOTE: if we already had a power of two, need to pad to the next one up
    // still because can't conditionally do padding = 0 B Float2D: 224 B -> add
    // 32 B -> 8 Ts Float3D: 288 B -> add 224 B -> 56 Ts Double2D: 448 B -> add
    // 64 B -> 8 Ts Double3D: 576 B -> add 448 B -> 56 Ts Vector<T, (48 * dim) -
    // 88> padding; //dim2 = 8 Ts, dim3 = 56 Ts --> y = 48x - 88
    ...
    // Vector<T, (-32 * dim) + 116> padding; //dim2 = 52 Ts, dim3 = 20 Ts --> y
    // = -32x + 116
    ...
    // Vector<T, (-16 * dim) + 80> padding; //dim2 = 48 Ts, dim3 = 32 Ts --> y =
    // -16x + 80
    ...
    // Vector<T, (92 * dim) - 152> padding; //dim2 = 32 Ts, dim3 = 124 Ts --> y
    // = 92x - 152
  }
  ...
  if (testcase == 201) {
    ...
    // fibrinogen density,          rho = 1395 g/cm^3 = 1,395,000 kg/m^3 -
    // https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3044599/

    using T = double;
    static const int dim = 2;
    MPM::CRAMPSimulator<T, dim> sim(
        "output/"
        "SENT_1e-3_noDamp_displacementBoundary_dx0.5mm_newTensorTransfer");
    ...
    // Params
    sim.dx = 0.5e-3; // 0.5 mm --> make sure this evenly fits into the width and
                     // height
    sim.symplectic = true;
    sim.end_frame = 4000;
    // sim.frame_dt = 22e-6 / sim.end_frame; //total time = 22e-6 s, want 1000
    // frames of this
    sim.frame_dt = 1e-3; // 1e-6 -> 1000 micro seconds total duration, 1e-3 -> 1
                         // second duration
    sim.gravity = 0;

    // Interpolation Scheme
    sim.useAPIC = false;
    sim.flipPicRatio = 0.0; // 0 -> want full PIC for analyzing static
                            // configurations (this is our damping)

    // DFG Specific Params
    sim.st = 4.0; // 4.0 is close to working, but we actually don't want
                  // surfacing for this demo
    sim.useDFG = true;
    sim.fricCoeff = 0.4;
    ...
    auto material1 =
        sim.create_elasticity(new MPM::FixedCorotatedOp<T, dim>(E, nu));
    // auto material1 = sim.create_elasticity(new MPM::LinearElasticityOp<T,
    // dim>(E, nu));
    ...
    sim.sampleGridAlignedBox(material1, minPoint, maxPoint,
                             Vector<T, dim>(0, 0), ppc, rho);
    // sim.samplGeridAlignedBoxWithPoissonDisk(material1, minPoint, maxPoint,
    // Vector<T, dim>(0, 0), ppc, rho);
    ...
    sim.addHorizontalCrack(Vector<T, dim>(crackX, crackY),
                           Vector<T, dim>(crackX + crackLength, crackY),
                           crackSegmentLength, damageRadius);

    // Add Boundary Conditions
    bool singlePuller = false;
    T yTop = y2 - 0.5e-3;
    T yBottom = y1 + 0.5e-3;
    T u2 = 0.2e-3; // pull a total displacement of 0.2 mm, so each puller will
                   // pull half this distance
    T pullTime = (sim.frame_dt * sim.end_frame) /
                 2.0; // pull for half of the total time duration
    ...
    if (singlePuller) {
      ...
      sim.add_boundary_condition(new Geometry::HalfSpaceLevelSet<T, dim>(
          Geometry::STICKY, Vector<T, dim>(0, yTop), Vector<T, dim>(0, -1),
          Vector<T, dim>(0, speed * 2.0), pullTime)); // top puller (pull up u2)
      sim.add_boundary_condition(new Geometry::HalfSpaceLevelSet<T, dim>(
          Geometry::STICKY, Vector<T, dim>(0, yBottom), Vector<T, dim>(0, 1),
          Vector<T, dim>(0, 0), pullTime)); // bottom puller (constant)
    } else {
      // pull from top and bottom, each pulling u2/2
      sim.add_boundary_condition(new Geometry::HalfSpaceLevelSet<T, dim>(
          Geometry::STICKY, Vector<T, dim>(0, yTop), Vector<T, dim>(0, -1),
          Vector<T, dim>(0, speed), pullTime)); // top puller (pull up u2/2)
      sim.add_boundary_condition(new Geometry::HalfSpaceLevelSet<T, dim>(
          Geometry::STICKY, Vector<T, dim>(0, yBottom), Vector<T, dim>(0, 1),
          Vector<T, dim>(0, -speed),
          pullTime)); // bottom puller (pull down u2/2)
    }
    ...
    T snapshotTime =
        sim.frame_dt *
        (sim.end_frame - 1); // 1950; //take snapshot after damping, around 1600
  ...
  if (testcase == 202) {
    ...
    // Simulation notes
    // Without damping, this sim comes to equilibrium within 2e-3 seconds (two
    // frames of 1e-3 frame dt), BUT there is still no crack opening
    // displacement
    ...
    // Using `new` to avoid redundant copy constructor
    // auto material1 = sim.create_elasticity(new MPM::FixedCorotatedOp<T,
    // dim>(E, nu));
    auto material1 =
        sim.create_elasticity(new MPM::LinearElasticityOp<T, dim>(E, nu));
    ...
    // sim.add_boundary_condition(new Geometry::HalfSpaceLevelSet<T,
    // dim>(Geometry::STICKY, Vector<T, dim>(0, 15e-3), Vector<T, dim>(0, -1)));
    ...
    T snapshotTime =
        sim.frame_dt * 7; // 1950; //take snapshot after damping, around 1600
    T halfEnvelope = sim.dx;
    sim.addStressSnapshot(snapshotTime, halfEnvelope);
  ...
  if (testcase == 203) {
    ...
    // DFG Specific Params
    sim.st = 0.0; // 4.0 is close to working, but we actually don't want
                  // surfacing for this demo
    sim.useDFG = true;
    sim.fricCoeff = 0.4;
    ...
    auto material1 =
        sim.create_elasticity(new MPM::FixedCorotatedOp<T, dim>(E, nu));
    sim.samplePrecutRandomCube(material1, Vector<T, dim>(0.4, 0.4),
                               Vector<T, dim>(0.6, 0.6), Vector<T, dim>(0, 0),
                               rho);
    sim.add_boundary_condition(new Geometry::HalfSpaceLevelSet<T, dim>(
        Geometry::STICKY, Vector<T, dim>(0, 0.59), Vector<T, dim>(0, -1)));
    sim.run(start_frame);
  }
  ...
  if (testcase == 204) {
    ...
    for (int i = 2; i < 6; ++i) {
      ...
      if (i == 4 || i == 5) {
        cleanString.erase(cleanString.find_last_not_of('0') + 1,
                          std::string::npos);
      }
      cleanedStrings.push_back(cleanString);
    }
    std::string path = "output/uniaxialTension_withDamage_Eta" +
                       cleanedStrings[0] + "_Zeta" + cleanedStrings[1] + "_p" +
                       cleanedStrings[2] + "_dMin" + cleanedStrings[3];
    ...
    sim.sampleGridAlignedBox(material1, minPoint, maxPoint,
                             Vector<T, dim>(0, 0), ppc, rho);
    // sim.sampleGridAlignedBoxWithPoissonDisk(material1, minPoint, maxPoint,
    // Vector<T, dim>(0, 0), ppc, rho);
    ...
    sim.addHorizontalCrackWithoutPoints(crackMin, crackMax, crackSegmentLength,
                                        damageRadius, crackType);
    ...
    T yTop = y2 - 0.5e-3;
    T yBottom = y1 + 0.5e-3;
    T u2 = 1.0e-3; // pull a total displacement of 0.2 mm, so each puller will
                   // pull half this distance
    T pullTime = (sim.frame_dt * sim.end_frame) /
                 2.0; // pull for half of the total time duration
    T speed = (u2 / 2.0) / pullTime;
  ...
  if (testcase == 205) {
    ...
    MPM::CRAMPSimulator<T, dim> sim(
        "output/planeStrainTension_E2.6e6_nu0.25_u2_1mm");
  ...
  if (testcase == 206) {
    ...
    MPM::CRAMPSimulator<T, dim> sim(
        "output/"
        "plateWithHole_E2.6e6_nu0.25_sigmaA_2600_aOverb0.06_dx0.25_"
        "smoothedTensors_nodalLoading");
    ...
    sim.sampleGridAlignedBoxWithHole(material1, minPoint, maxPoint,
                                     Vector<T, dim>(center, center), radius,
                                     Vector<T, dim>(0, 0), ppc, rho);
    // sim.sampleGridAlignedBoxWithPoissonDisk(material1, minPoint, maxPoint,
    // Vector<T, dim>(0, 0), ppc, rho);
    ...
    // T u2 = 1.0e-3; // pull a total displacement of 0.2 mm, so each puller
    // will pull half this distance T pullTime = (sim.frame_dt * sim.end_frame)
    // / 2.0; //pull for half of the total time duration T speed = (u2 / 2.0) /
    // pullTime; std::cout << "speed:" << speed << std::endl;
    ...
    // T yTop = y2 - 0.5e-3;
    // T yBottom = y1 + 0.5e-3;
    // sim.addMode1Loading(yTop, yBottom, sigmaA, rampTime, false); //particle
    // loading
    sim.addMode1Loading(y2, y1, sigmaA, rampTime, true, width, x1,
                        x2); // if doing nodal loading, pass y1, y2, x1, x2 as
                             // the exact min and max of the material!

    // if(singlePuller){
    //     //fix bottom constant, pull on top the full u2
    //     sim.add_boundary_condition(new Geometry::HalfSpaceLevelSet<T,
    //     dim>(Geometry::STICKY, Vector<T, dim>(0, yTop), Vector<T, dim>(0,
    //     -1), Vector<T, dim>(0, speed * 2.0), pullTime)); //top puller (pull
    //     up u2) sim.add_boundary_condition(new Geometry::HalfSpaceLevelSet<T,
    //     dim>(Geometry::STICKY, Vector<T, dim>(0, yBottom), Vector<T, dim>(0,
    //     1), Vector<T, dim>(0, 0), pullTime)); //bottom puller (constant)
    // }
    // else{
    //     //pull from top and bottom, each pulling u2/2
    //     sim.add_boundary_condition(new Geometry::HalfSpaceLevelSet<T,
    //     dim>(Geometry::STICKY, Vector<T, dim>(0, yTop), Vector<T, dim>(0,
    //     -1), Vector<T, dim>(0, speed), pullTime)); //top puller (pull up
    //     u2/2) sim.add_boundary_condition(new Geometry::HalfSpaceLevelSet<T,
    //     dim>(Geometry::STICKY, Vector<T, dim>(0, yBottom), Vector<T, dim>(0,
    //     1), Vector<T, dim>(0, -speed), pullTime)); //bottom puller (pull down
    //     u2/2)
    // }
