...
template <class T, int dim> class CRAMPSimulator : public MPMSimulator<T, dim> {
public:
  ...
  /* We have these from inheriting from MPMSimulator, use them prefixed with
  Base:: Field<TV> m_X; Field<TV> m_V; Field<TM> m_C; std::vector<T> m_mass;
  Field<TM> stress;
  MPMGrid<T, dim> grid;
  T dx = 0.02;
  TV gravity = TV::Zero();
  ...
  Field<int> m_marker; // 0 = material particle, 1 = crack particle, 2 = top
                       // plane, 3 = bottom plane
  int crackPlane_startIdx;
  int topPlane_startIdx;
  int bottomPlane_startIdx;
  int bottomPlane_endIdx; // only need this one end idx, other end ideces
                          // defined by the next start idx
  int crackType;

  // Mode I Loading Params - pull up on nodes above y1, pull down below y2, all
  // with total stress sigmaA
  T y1, y2;
  T sigmaA;
  T rampTime;
  T width;
  ...
  Field<std::vector<int>>
      particleAF; // store which activefield each particle belongs to for the
                  // 3^d grid nodes it maps to
  Field<std::vector<int>> p_cached_idx; // store which DOF index each particle
                                        // maps to for each of the 3^dim nodes
  ...
  void initialize() {
    ...
    Bow::Logging::info("Simulation starts with ",
    ...
    if (useDamage) {
      ...
      if (percentStretch > 0) {
        for (auto &model : Base::elasticity_models)
          model->compute_criticalStress(
              percentStretch, Base::stress); // use stress as a dummy structure
                                             // here to hold stretchedCauchy
        ...
        tbb::parallel_for(size_t(0), sigmaC.size(),
                          [&](size_t i) { sigmaC[i] = sigmaCRef; });
        Bow::Logging::info("[AnisoMPM Damage] Directly Set SigmaC: ",
                           sigmaC[0]);
      }
    }
  ...
  void partitioningRoutines() {
    ...
    Bow::DFGMPM::NeighborSortOp<T, dim> neighbor_sort{
        {}, Base::m_X, particleNeighbors, grid, rp};
    neighbor_sort(); // Create neighbor list for each particle

    // Now, with particle neighbor lists in hand, we need to resort into a grid
    // with dx = dx
    grid.sortParticles(Base::m_X, Base::dx);

    // Surface Detection -> only on first substep
    if (elapsedTime == 0.0) {
      Bow::DFGMPM::SurfaceDetectionOp<T, dim> surface_detection{
          {}, Base::m_X, particleNeighbors, rp, st, sp, grid};
    ...
    if (useRankineDamage) {
      ...
      // NOTE: we can put rankine damage here because, unlike AnisoMPM damage,
      // we can update this BEFORE computing DGs!
    }

    Bow::DFGMPM::ComputeDamageGradientsOp<T, dim> compute_DGs{
        {}, Base::m_X, particleNeighbors, rp, Base::dx, particleDG, Dp,
        sp, grid};
    compute_DGs(); // Compute particle damage gradients

    Bow::DFGMPM::PartitioningOp<T, dim> partition{
        {}, Base::m_X, Base::m_mass, particleDG, particleAF, Dp,
        sp, Base::dx,  minDp,        dMin,       grid};
    partition(); // Partition particles into their fields, transfer mass to
                 // those fields, and compute node separability
  }

  // AnisoMPM Routines
  void anisoMPMDamage(T dt) {
    Bow::DFGMPM::ComputeDamageLaplaciansOp<T, dim> compute_damageLaplacians{
        {}, Base::m_X, Dp, damageLaplacians, particleAF, Base::dx, grid};
    compute_damageLaplacians();

    // for (auto& model : Base::elasticity_models) //compute cauchy stress to
    // pass to next method
    //     model->compute_cauchy(m_cauchy); //NOTE: already do this now in main
    //     loop

    Bow::DFGMPM::UpdateAnisoMPMDamageOp<T, dim> update_anisoMPM_damage{
        {}, Dp,  damageLaplacians, dTildeH, sigmaC, m_cauchy, dt, eta, zeta,
        l0, grid};
  ...
  void p2g(T dt) {
    if (!useDFG) {
      // if single field MPM, sort particles before P2G (for two field we
      // already did this)
      grid.sortParticles(Base::m_X, Base::dx);
    }
    if (Base::symplectic) {
    ...
    // else if(useImplicitContact){ //grab mu, la, and vol for barrier eval --
    // TAG=BARRIER
    //     for (auto& model : Base::elasticity_models){
    //         m_mu = model->m_mu;
    //         m_la = model->m_lambda;
    ...
    Bow::CRAMP::ParticlesToGridOp<T, dim> P2G{
        {},           Base::m_X,  Base::m_V,
        Base::m_mass, Base::m_C,  Base::stress,
        gravity,      particleAF, grid,
        Base::dx,     dt,         Base::symplectic,
        useDFG,       useAPIC,    useImplicitContact};
    P2G();
  }
  ...
  void gridUpdate(T dt) {
    if (Base::symplectic) {
      ...
      // Boundary Collisions
      Bow::DFGMPM::BoundaryConditionUpdateOp<T, dim> bc_update{
          {}, grid, Base::BC, Base::dx, dt};
      bc_update();
    }
    // else{
    //     Bow::DFGMPM::ImplicitBoundaryConditionUpdateOp<T, dim> bc_update{ {},
    //     grid, Base::BC, Base::BC_basis, Base::BC_order, Base::dx, useDFG };
    //     bc_update();
    //     Bow::DFGMPM::TwoFieldHodgepodgeEnergy<T, dim> energy(grid, gravity,
    //     Base::dx, dt, Base::m_X, Base::elasticity_models, particleAF,
    //     p_cached_idx, useDFG, useImplicitContact);
    //     Bow::DFGMPM::TwoFieldBackwardEulerUpdateOp<T, dim, int>
    //     implicit_mpm(grid, Base::BC_basis, Base::BC_order, Base::dx, dt,
    //     useDFG); implicit_mpm.m_energy_terms.push_back(&energy); if
    //     (Base::backward_euler) {
    //         energy.tsMethod = BE;
  ...
  void g2p(T dt) {
    Bow::DFGMPM::GridToParticlesOp<T, dim> G2P{
        {},   Base::m_X, Base::m_V, Base::m_C,    particleAF,
        grid, Base::dx,  dt,        flipPicRatio, useDFG};
    G2P(useAPIC); // P2G
  ...
  void advance(T dt) override {
    ...
    for (auto &model : Base::elasticity_models) {
      model->compute_cauchy(
          m_cauchy); // we also use this for anisoMPM damage --> do not take out
                     // unless replace it in AnisoMPM damage
      m_F = model->m_F;
    ...
    if (useDFG) {
      ...
      if (useAnisoMPMDamage) {
        anisoMPMDamage(dt); // note that we simply update and track damage,
                            // there is no elasticity deg
    ...
    // Now transfer cauchy and F to the grid (requires grid masses, so, after
    // P2G)
    Bow::CRAMP::TensorP2GOp<T, dim> tensorP2G{{},       Base::m_X, Base::m_mass,
                                              m_cauchy, m_F,       particleAF,
                                              grid,     Base::dx,  useDFG};
    tensorP2G();
    std::cout << "Tensor P2G Done..." << std::endl;
    Bow::CRAMP::TensorG2POp<T, dim> tensorG2P{
        {},         Base::m_X, m_cauchySmoothed, m_FSmoothed,
        particleAF, grid,      Base::dx,         useDFG};
    tensorG2P();
    ...
    if (takeStressSnapshot && elapsedTime >= stressSnapshotTime) {
      ...
      Vector<T, dim> crackTip =
          Base::m_X[topPlane_startIdx - 1]; // crack tip should be last massless
                                            // particle before topPlane particles
      Bow::CRAMP::StressSnapshotOp<T, dim> stressSnapshot{
          {},        Base::m_X, crackTip, m_cauchy, grid,        Base::dx,
          m_sigmaYY, m_r,       m_posX,   m_idx,    halfEnvelope};
      stressSnapshot();
      writeStressSnapshot(elapsedTime);

      // Collect grid forces, grid nominal stress (P), and grid def grad (F) --
      // all to compute J-Integral!
      for (auto &model : Base::elasticity_models) {
        m_mu = model->m_mu;
        m_la = model->m_lambda;
      }
      Bow::CRAMP::CollectJIntegralGridDataOp<T, dim> collectJIntegralGridData{
          {},   Base::m_X, Base::stress, m_F,   particleAF,
          grid, Base::dx,  dt,           useDFG};
      collectJIntegralGridData();

      // As part of the stress snapshot let's also compute the J-integral!
      // We will compute the J integral using however many contour radii the
      // user asks for
      std::string jIntFilePath =
          outputPath + "/JIntegralData" + std::to_string(elapsedTime) + ".txt";
      std::ofstream jIntFile(jIntFilePath);
      Bow::CRAMP::ComputeJIntegralOp<T, dim> computeJIntegral{
          {},
          Base::m_X,
          crackTip,
          topPlane_startIdx,
          bottomPlane_startIdx,
          m_cauchy,
          grid,
          Base::dx,
          dt,
    ...
    if (loading) {
      ...
      if (!particlesMarkedForLoading) {
        Bow::CRAMP::MarkParticlesForLoadingOp<T, dim> markParticles{
            {}, Base::m_X, m_marker, y1, y2, grid};
        markParticles();
        particlesMarkedForLoading = true;
      }

      // Pass the right portion of sigmaA to the loading (based on the user
      // defined rampTime)
      T scaledSigmaA = sigmaA;
      if (elapsedTime < rampTime && rampTime > 0.0) {
        scaledSigmaA *= (elapsedTime / rampTime);
      }
      Bow::CRAMP::ApplyMode1LoadingOp<T, dim> mode1Loading{
          {}, Base::m_X, m_marker, scaledSigmaA, nodalLoading, width, y1, y2,
          x1, x2,        Base::dx, dt,           grid,         m_vol, ppc};
      mode1Loading();

      std::cout << "Mode 1 Loading Applied..." << std::endl;
    }

    // Apply Impulse (if user added one) -> apply directly for symplectic, save
    // forces for later if implicit
    if (useImpulse) {
      if ((elapsedTime >= impulseStartTime) &&
          (elapsedTime < impulseStartTime + impulseDuration)) {
        Bow::DFGMPM::ApplyImpulseOp<T, dim> apply_impulse{
            {}, impulseCenter,    impulseStrength, grid, Base::dx,
            dt, Base::symplectic, useImpulse};
        apply_impulse();
        std::cout << "Impulse Applied..." << std::endl;
      }
    }

    // Frictional Contact -> apply directly for symplectic, for implicit we
    // compute normals here (but only if we want implicit contact)
    if (useDFG &&
        (Base::symplectic || (!Base::symplectic && useImplicitContact))) {
      Bow::DFGMPM::ContactForcesOp<T, dim> frictional_contact{
          {}, dt, fricCoeff, Base::symplectic, useImplicitContact, grid};
      frictional_contact();
    ...
    // Explicit Frictional Contact -> ONLY for implicit two field with EXPLICIT
    // frictional contact
    ...
    if (useSimpleDamping && elapsedTime < simpleDampingDuration) {
      // Use simple damping
      Bow::CRAMP::SimpleDampingOp<T, dim> applySimpleDamping{
          {}, Base::m_V, simpleDampingFactor, grid};
      applySimpleDamping();
      std::cout << "Simple Damping Applied..." << std::endl;
    }

    if (crackInitialized) {
      Bow::CRAMP::EvolveCrackPlanesOp<T, dim> evolve_cracks{
          {},
          Base::m_X,
          Base::m_V,
          topPlane_startIdx,
    ...
    // Now dump substep data if verbose is active (verbose = write every
    // substep)
    if (verbose) {
      BOW_TIMER_FLAG("writeSubstep");

      IO::writeTwoField_particles_ply(
          outputPath + "/p" + std::to_string(currSubstep) + ".ply", Base::m_X,
          Base::m_V, particleDG, Base::m_mass, Dp, sp, m_marker,
          m_cauchySmoothed, m_FSmoothed);

      std::cout << "Substep Written..." << std::endl;

      // Write Grid
      if (writeGrid) {
        Bow::DFGMPM::CollectGridDataOp<T, dim> collect_gridData{
            {},
            grid,
            Base::dx,
            activeNodesX,
            ...
            activeNodesM1,
            activeNodesM2,
            activeNodesSeparability1,
            activeNodesSeparability2,
            activeNodesSeparable};
        collect_gridData();
        IO::writeTwoField_nodes_ply(
            outputPath + "/i" + std::to_string(currSubstep) + ".ply",
            activeNodesX, activeNodesCauchy1, activeNodesCauchy2,
            activeNodesFi1, activeNodesFi2, activeNodesDG, activeNodesV1,
            activeNodesV2, activeNodesFct1, activeNodesFct2, activeNodesM1,
            activeNodesM2, activeNodesSeparability1, activeNodesSeparability2,
            activeNodesSeparable);
      }
    }
  }
  ...
  // Setup sim for AnisoMPM Damage -- NOTE: if you want to set sigmaC directly,
  // pass p < 0 and your sigmaC
  void addAnisoMPMDamage(T _eta, T _dMin, T _zeta, T _p = -1.0,
                         T _sigmaC = -1.0) {
    assert(_p > 0 ^ _sigmaC > 0); // assert that exactly one of these is set
    assert(!useDamage); // if we've already added a damage model we can't add
                        // another!
  ...
  // Setup a mode I constant loading for the configuration, we pull up on nodes
  // above y1, and pull down on nodes below y2, each with total stress sigmaA.
  void addMode1Loading(T _y1, T _y2, T _sigmaA, T _rampTime, bool _nodalLoading,
                       T _width = 0, T _x1 = 0, T _x2 = 0) {
    if (_nodalLoading) {
      BOW_ASSERT_INFO(
          _width > 0,
          "ERROR: Nodal loading requires passing the specimen width");
    }
    y1 = _y1;
    y2 = _y2;
    sigmaA = _sigmaA;
  ...
  T suggestedDt(T _E, T _nu, T _rho, T _dx, T _cfl) {
    T elasticity =
        std::sqrt(_E * (1 - _nu) / ((1 + _nu) * (1 - 2 * _nu) * _rho));
    return _cfl * _dx / elasticity;
  }

  // Ensure that our dt never allows a particle to move faster than one grid
  // cell per step
  virtual T calculate_dt() {
    T max_speed = 0;
    for (int i = 0; i < (int)Base::m_X.size(); ++i) {
      max_speed =
          Base::m_V[i].norm() > max_speed ? Base::m_V[i].norm() : max_speed;
    }
  ...
  void dump_output(int frame_num) override {
    if (!frame_num || !verbose) {
      ...
      IO::writeTwoField_particles_ply(
          outputPath + "/p" + std::to_string(frame_num) + ".ply", Base::m_X,
          Base::m_V, particleDG, Base::m_mass, Dp, sp, m_marker,
          m_cauchySmoothed, m_FSmoothed);
      ...
      if (writeGrid) {
        ...
        collect_gridData();
        IO::writeTwoField_nodes_ply(
            outputPath + "/i" + std::to_string(frame_num) + ".ply",
            activeNodesX, activeNodesCauchy1, activeNodesCauchy2,
            activeNodesFi1, activeNodesFi2, activeNodesDG, activeNodesV1,
            activeNodesV2, activeNodesFct1, activeNodesFct2, activeNodesM1,
            activeNodesM2, activeNodesSeparability1, activeNodesSeparability2,
  ...
  void writeStressSnapshot(T elapsedTime) {
    std::string filepath =
        outputPath + "/stressSnapshot" + std::to_string(elapsedTime) + ".csv";
  ...
  // Add simple damping routine (multiply particle velocities after G2P by
  // dampFactor)
  ...
  void sampleRandomCube(std::shared_ptr<ElasticityOp<T, dim>> model,
                        const TV &min_corner, const TV &max_corner,
                        const TV &velocity = TV::Zero(), T density = 1000.) {
    // sample particles
    T vol =
        dim == 2 ? Base::dx * Base::dx / 4 : Base::dx * Base::dx * Base::dx / 8;
    T interval = Base::dx / std::pow((T)ppc, (T)1 / dim);
    Vector<int, dim> region =
        ((max_corner - min_corner) / interval).template cast<int>();
  ...
  void samplePrecutRandomCube(std::shared_ptr<ElasticityOp<T, dim>> model,
                              const TV &min_corner, const TV &max_corner,
                              const TV &velocity = TV::Zero(),
                              T density = 1000.) {
    // sample particles
    // T vol = dim == 2 ? Base::dx * Base::dx / 4 : Base::dx * Base::dx *
    // Base::dx / 8;
    ...
    iterateRegion(region, [&](const Vector<int, dim> &offset) {
      ...
      if ((position[1] <= y1 + buffer) && (position[1] >= y1 - buffer) &&
          (position[0] >= x1)) {
        Dp.push_back(1.0);
      } else if ((position[1] <= y2 + buffer) && (position[1] >= y2 - buffer) &&
                 (position[0] <= x2)) {
        Dp.push_back(1.0);
      } else {
        Dp.push_back(0.0);
  ...
  void sampleRandomSphere(std::shared_ptr<ElasticityOp<T, dim>> model,
                          const TV &center, const T radius,
                          const TV &velocity = TV::Zero(), T density = 1000.) {
    // sample particles
    T vol =
        dim == 2 ? Base::dx * Base::dx / 4 : Base::dx * Base::dx * Base::dx / 8;
  ...
  void sampleFromObj(std::shared_ptr<ElasticityOp<T, dim>> model,
                     const std::string filepath, const TV &velocity, T volume,
                     T density) {
    ...
    // Know vertex is always preceded by 'v' and these are the only lines we
    // care about
    std::string line;
    TV position;
  ...
  // NOTE: This routine works best if the dimensions of the box are even
  // multiples of the grid resolution (width = c1 * dx, height = c2 * dx)
  void sampleGridAlignedBox(std::shared_ptr<ElasticityOp<T, dim>> model,
                            const TV &min_corner, const TV &max_corner,
                            const TV &velocity = TV::Zero(), int _ppc = 4,
                            T density = 1000.) {
    BOW_ASSERT_INFO(min_corner != max_corner,
                    "min_corner == max_corner in sampleGridAlignedBox");
    ...
    // BOW_ASSERT_INFO(widthMod == Base::dx || widthMod == 0, "width not
    // divisible by dx in sampleGridAlignedBox"); BOW_ASSERT_INFO(heightMod ==
    // Base::dx || heightMod == 0, "height not divisible by dx in
    // sampleGridAlignedBox");
    ...
    iterateRegion(region, [&](const Vector<int, dim> &offset) {
      ...
      Dp.push_back(0.0);
      damageLaplacians.push_back(0.0);
      if (offset[0] == 0 || offset[1] == 0 || offset[0] == region[0] - 1 ||
          offset[1] == region[1] - 1) {
        sp.push_back(1);
      } else {
        sp.push_back(0);
  ...
  // NOTE: This routine works best if the dimensions of the box are even
  // multiples of the grid resolution (width = c1 * dx, height = c2 * dx)
  void sampleGridAlignedBoxWithHole(std::shared_ptr<ElasticityOp<T, dim>> model,
                                    const TV &min_corner, const TV &max_corner,
                                    const TV &center, const T radius,
                                    const TV &velocity = TV::Zero(),
                                    int _ppc = 4, T density = 1000.) {
    BOW_ASSERT_INFO(min_corner != max_corner,
                    "min_corner == max_corner in sampleGridAlignedBox");
    // T width = max_corner[0] - min_corner[0];
    // T height = max_corner[1] - min_corner[1];
  ...
  void sampleGridAlignedBoxWithPoissonDisk(
      std::shared_ptr<ElasticityOp<T, dim>> model, const TV &min_corner,
      const TV &max_corner, const TV &velocity = TV::Zero(), int _ppc = 4,
      T density = 1000.) {
    // sample particles
    ppc = (T)_ppc;
    T vol = std::pow(Base::dx, dim) / T(_ppc);
    ...
    Geometry::PoissonDisk<T, dim> poisson_disk(min_corner, max_corner, Base::dx,
                                               T(_ppc));
  ...
  void addHorizontalCrack(const TV &minPoint, const TV &maxPoint, T increment,
                          T radius, int _crackType = 0) {
    // Crack type: 0 = left side, 1 = middle, 2 = right side
    crackType = _crackType;
    BOW_ASSERT(dim == 2);
    T region = (maxPoint(0) - minPoint(0)) / increment;
    crackPlane_startIdx = Base::m_X.size(); // start idx for crack path
                                            // particles
    ...
    topPlane_startIdx =
        Base::m_X.size(); // now add duplicate particles for the top plane
    for (int i = 0; i < region; i++) {
    ...
    bottomPlane_startIdx =
        Base::m_X.size(); // now add duplicate particles for the bottom plane
    for (int i = 0; i < region; i++) {
    ...
    grid.crackParticlesStartIdx =
        crackPlane_startIdx; // we'll use this in DFGMPMOp to make sure we don't
                             // access these particles for other operations!
    ...
    for (int i = 0; i < crackPlane_startIdx;
         i++) { // iter normal material particles
      TV p = Base::m_X[i];
      // TV crackTip = Base::m_X[crackTipIdx];
      for (int j = crackPlane_startIdx; j < topPlane_startIdx;
           j++) { // check material particles against every crack plane particle
        TV c = Base::m_X[j];
        T dX = p(0) - c(0);
        T dY = p(1) - c(1);
  ...
  void addHorizontalCrackWithoutPoints(const TV &minPoint, const TV &maxPoint,
                                       T increment, T radius,
                                       int _crackType = 0) {
    // Crack type: 0 = left side, 1 = middle, 2 = right side
    crackType = _crackType;
    BOW_ASSERT(dim == 2);
    ...
    // Now we need to mark particles near the crack as fully damaged
    for (int i = 0; i < (int)Base::m_X.size();
         i++) { // iter normal material particles
      TV p = Base::m_X[i];
      for (int j = 0; j < (int)crackPoints.size();
           j++) { // check material particles against every crack plane particle
        TV c = crackPoints[j];
        T dX = p(0) - c(0);
        T dY = p(1) - c(1);
