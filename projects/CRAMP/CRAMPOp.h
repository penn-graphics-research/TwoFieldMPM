...
/* Transfer particle quantities to the grid as well as explicit update for
 * velocity*/
template <class T, int dim> class ParticlesToGridOp : public AbstractOp {
public:
  ...
  void operator()() {
    ...
    grid.colored_for([&](int i) {
      if (!grid.crackInitialized ||
          i < grid.crackParticlesStartIdx) { // skip crack particles if we have
                                             // them
        ...
        const Matrix<T, dim, dim> delta_t_tmp_force =
            -dt * stress[i]; // stress holds Vp^0 * PF^T
        BSplineWeights<T, dim> spline(pos, dx);

        // Grab volume
        // T vol = m_vol[i];

        grid.iterateKernel(spline, [&](const Vector<int, dim> &node, int oidx,
                                       T w, const Vector<T, dim> &dw,
                                       DFGMPM::GridState<T, dim> &g) {
          ...
          if (symplectic) {
            stress_term_dw =
                delta_t_tmp_force *
                dw; // only add forces if symplectic, else added in residual
          }
          Vector<T, dim> delta_APIC = w * velocity_term_APIC + stress_term_dw;
          Vector<T, dim> delta_FLIP = w * momentum + stress_term_dw;
          Vector<T, dim> delta_vn =
              w * momentum; // we'll use this to compute v1^n and v2^n for FLIP

          // Notice we treat single-field and two-field nodes differently
          // NOTE: remember we are also including explicit force here if
          // symplectic!
          if (g.separable != 1 || !useDFG) {
            // Single-field treatment if separable = 0 OR if we are using single
            // field MPM
            if (useAPIC) {
              g.v1 += delta_APIC;
            } else {
            ...
            // Now transfer mass if we aren't using DFG (this means we skipped
            // massP2G earlier)
            if (!useDFG) {
              g.m1 += mass * w;
            }
            ...
            int fieldIdx =
                particleAF[i][oidx]; // grab the field that this particle
                                     // belongs in for this grid node (oidx)
            if (fieldIdx == 0) {
              if (useAPIC) {
                g.v1 += delta_APIC;
              } else {
    ...
    /* Iterate grid to divide out the grid masses from momentum and then add
     * gravity */
    grid.iterateGrid(
        [&](const Vector<int, dim> &node, DFGMPM::GridState<T, dim> &g) {
          Vector<T, dim> gravity_term = Vector<T, dim>::Zero();
          if (symplectic) {
            gravity_term[1] =
                gravity * dt; // only nonzero if symplectic, else we add gravity
                              // in residual of implicit solve
          }
          T mass1 = g.m1;
          Vector<T, dim> alpha1;
          ...
          g.x1 = node.template cast<T>() *
                 dx; // put nodal position in x1 regardless of separability
          if (g.separable == 1) {
            T mass2 = g.m2;
            Vector<T, dim> alpha2;
...
template <class T, int dim> class TensorP2GOp : public AbstractOp {
public:
  using SparseMask = typename DFGMPM::DFGMPMGrid<T, dim>::SparseMask;
  Field<Vector<T, dim>> &m_X;
  std::vector<T> &m_mass;
  ...
  void operator()() {
    ...
    grid.colored_for([&](int i) {
      if (!grid.crackInitialized ||
        ...
        const Vector<T, dim> pos = m_X[i];
        const T mass = m_mass[i];
        const Matrix<T, dim, dim> cauchyXmass =
            m_cauchy[i] * mass;                                 // cauchy * m_p
...
/* Transfer Cauchy stress and deformation gradient back from the grid for a
 * smoothed cauchy and F! */
template <class T, int dim> class TensorG2POp : public AbstractOp {
public:
  using SparseMask = typename DFGMPM::DFGMPMGrid<T, dim>::SparseMask;
  Field<Vector<T, dim>> &m_X;
...
/* CRAMP Partitioned P2G: for each particle to grid mapping, check whether the
   line between them crosses the explicit crack path if it does not, transfer
   particle quantity to field 1 and separable = 0 if it does, transfer particle
   quantity either to field 1 or 2 (depending on CRAMP partitioning algorithm),
   and separable = 1 NOTE: we also need to set particleAF so we can map
   backwards in G2P */
template <class T, int dim> class CRAMPPartitionedP2GOp : public AbstractOp {
public:
  using SparseMask = typename DFGMPM::DFGMPMGrid<T, dim>::SparseMask;
  Field<Vector<T, dim>> &m_X;
  Field<Vector<T, dim>> &m_V;
  ...
  void operator()() {
    ...
    grid.colored_for([&](int i) {
      if (!grid.crackInitialized ||
        ...
        grid.iterateKernel(spline, [&](const Vector<int, dim> &node, int oidx,
          ...
          // Now we need to determine for this particle/node pairing whether we
          // should transfer to field 1 or 2 and whether this node is separable
          // or not.

          // SUBTASK 1: determine if rectangle defined by x1 and x2 intersects
          ...
          if (intersectCrack) {
            ...
            // SUBTASK 2/3: for each crack segment with endpoints x3 and x4,
            // compute the signs of the areas of triangles 123, 124, 341, and 342
            for (int j = grid.crackParticlesStartIdx; j < topPlane_startIdx - 1;
                 j++) {
              ...
              if (a1 == 1 && a2 == 1) {
                intersectCrack =
                    false; // if these are +,+ there is never an intersect
              } else if (a1 == -1 && a2 == -1) {
                ...
                numCrossings++; // pre-emptively increase this, we will decrease
                                // it if there was no intersection
            ...
            // SUBTASK 3/4: determine whether there was an intersection or not
            // based on crossing count
            if (numCrossings % 2 == 0) { // if number of crossings was even,
                                         // ignore the intersections
              intersectCrack = false;
            }
          }

          // TODO: if we want CRAMP partitioning, we need to completely change
          // some other parts of the MPM data flow because it requires THREE
          // fields, not just two
          // //Now, set separable, particleAF, and do two field P2G all based on
          // intersectCrack and above/belowCrack! if(intersectCrack){
          ...
          //     }
          //     else{
          //         std::cout << "ERROR: Shouldn't get here... :c" <<
          //         std::endl;
          //     }
          // }
          // else{
          ...
          // //Notice we treat single-field and two-field nodes differently
          // //NOTE: remember we are also including explicit force here if
          // symplectic! if (g.separable != 1 || !useDFG) {
          //     //Single-field treatment if separable = 0 OR if we are using
          //     single field MPM if (useAPIC) {
          //         g.v1 += delta_APIC;
          //     }
          //     else {
          //         g.v1 += delta_FLIP;
          ...
          //     //Now transfer mass if we aren't using DFG (this means we
          //     skipped massP2G earlier) if (!useDFG) {
          //         g.m1 += mass * w;
          //     }
          ...
          // else if (g.separable == 1 && useDFG) {
          //     //Treat node as having two fields
          //     int fieldIdx = particleAF[i][oidx]; //grab the field that this
          //     particle belongs in for this grid node (oidx) if (fieldIdx ==
          //     0) {
          //         if (useAPIC) {
          //             g.v1 += delta_APIC;
          //         }
    ...
    // /* Iterate grid to divide out the grid masses from momentum and then add
    // gravity */ grid.iterateGrid([&](const Vector<int, dim>& node,
    // DFGMPM::GridState<T, dim>& g) {
    //     Vector<T, dim> gravity_term = Vector<T, dim>::Zero();
    //     if (symplectic) {
    //         gravity_term[1] = gravity * dt; //only nonzero if symplectic,
    //         else we add gravity in residual of implicit solve
    //     }
    //     T mass1 = g.m1;
    //     Vector<T, dim> alpha1;
    //     alpha1 = Vector<T, dim>::Ones() * ((T)1 / mass1);
    ...
    //     g.v1 += gravity_term;
    //     g.vn1 = g.vn1.cwiseProduct(alpha1); // this is how we get v1^n
    //     g.x1 = node.template cast<T>() * dx; //put nodal position in x1
    //     regardless of separability if (g.separable == 1) {
    //         T mass2 = g.m2;
    //         Vector<T, dim> alpha2;
    //         alpha2 = Vector<T, dim>::Ones() * ((T)1 / mass2);
    //         g.v2 = g.v2.cwiseProduct(alpha2);
...
int signedTriangleArea(Vector<T, dim> _x1, Vector<T, dim> _x2,
                       Vector<T, dim> _x3) {
  T x1 = _x1[0];
  T x2 = _x2[0];
  T x3 = _x3[0];
...
/*Mark particles for loading, takes in upper and lower boundaries for Mode 1
 * loading */
template <class T, int dim>
    ...
    grid.parallel_for([&](int i) {
      if (!grid.crackInitialized ||
          i < grid.crackParticlesStartIdx) { // skip crack particles if we have
                                             // them
        const Vector<T, dim> pos = m_X[i];
        if (pos[1] > y1 || pos[1] < y2) { // ONLY set to 4 or 5 if particle is
                                          // above y1 and below y2!
          m_marker[i] = 4;
          if (pos[1] < y2) {
...
/*Iterate grid to apply a mode I loading to the configuration based on y1, y2,
 * and sigmaA */
template <class T, int dim> class ApplyMode1LoadingOp : public AbstractOp {
public:
  using SparseMask = typename DFGMPM::DFGMPMGrid<T, dim>::SparseMask;
  Field<Vector<T, dim>> &m_X;
  Field<int> m_marker;
  ...
  void operator()() {
    ...
    if (!nodalLoading) {
      // Using particle volume, compute per particle forces using the
      // scaledSigmaA passed in, then transfer this force to the grid and apply
      // it
      grid.colored_for([&](int i) {
        if (!grid.crackInitialized ||
            i < grid.crackParticlesStartIdx) { // skip crack particles if we
                                               // have them
          const Vector<T, dim> pos = m_X[i];
          const int marker = m_marker[i];
          stress = scaledSigmaA;

          if (marker == 4 || marker == 5) { // ONLY apply this force to
                                            // particles above y1 and below y2!
            if (marker == 5) {
            ...
            T fp = Ap * stress; // particle force (working simply with y
                                // direction magnitude, not full vector)

            // std::cout << "particle idx:" << i << "fp:" << fp << std::endl;

            BSplineWeights<T, dim> spline(pos, dx);
            grid.iterateKernel(spline,
                               [&](const Vector<int, dim> &node, int oidx, T w,
                                   const Vector<T, dim> &dw,
                                   DFGMPM::GridState<T, dim> &g) {
                                 // Store f_i in gridViYi since we're not using
                                 // it anyway
                                 g.gridViYi1 += fp * w;
                               });
          }
      ...
      // F_nodal = F_total / N = (sigmaA * width * thickness) / (width / dx) ->
      // we use t = 1 here F_nodal = sigmaA * thickness * dx -> t = 1
    ...
    grid.iterateGrid(
        [&](const Vector<int, dim> &node, DFGMPM::GridState<T, dim> &g) {
          ...
          if (!nodalLoading) {
            ...
            // T endX2 = x2 + eps;
            if (xi[0] > midX1 &&
                xi[0] < midX2) { // only within the x range of the material!
              if (std::abs(xi[1] - y1) < eps) { // top pulls up
                fi = stress;
              } else if (std::abs(xi[1] - y2) < eps) { // bottom pulls down
                fi = -1 * stress;
              }
            } else if (std::abs(xi[0] - x1) < eps ||
                       std::abs(xi[0] - x2) < eps) { // end nodes
              if (std::abs(xi[1] - y1) < eps) {      // top pulls up
                fi = 0.5 * stress;
              } else if (std::abs(xi[1] - y2) < eps) { // bottom pulls down
                fi = -0.5 * stress;
          ...
          g.v1[1] +=
              (fi / g.m1) * dt; // update velocity based on our computed forces
        });
  }
};

/* Evolve crack planes -- we need to use v1, v2, and v_cm and do a G2P style
 * transfer from nodes to crack plane particles, then perform a simple particle
 * advection */
template <class T, int dim> class EvolveCrackPlanesOp : public AbstractOp {
public:
  using SparseMask = typename DFGMPM::DFGMPMGrid<T, dim>::SparseMask;
  ...
  void operator()() {
    // First let's iterate separable grid nodes and compute v_cm and store it in
    // fi1 (since we won't apply dynamic impulses for cramp)
    BOW_TIMER_FLAG("evolveCrackPlanes");

    // set some indeces so we can always set the top and bottom plane tips to be
    // equal to the central crack tip
    ...
    grid.iterateGrid(
        [&](const Vector<int, dim> &node, DFGMPM::GridState<T, dim> &g) {
          // For separable nodes, compute the frictional contact forces for each
          // field
          if (g.separable == 1) {
            // Grab momentum
            ...
            Vector<T, dim> v_cm =
                q_cm /
                m_cm; // THIS IS CRUCIAL we need to compute v_cm using momentum
    ...
    grid.parallel_for([&](int i) {
      if (i >= grid.crackParticlesStartIdx &&
          i < topPlane_startIdx) { // only process the crack plane particles
        ...
        grid.iterateKernel(spline, [&](const Vector<int, dim> &node, int oidx,
          ...
          if (g.separable != 1) {
            ...
            // treat as two-field node -> use our computed v_cm (new and old)
            // values in fi1 and fi2
            picV += w * g.fi1;
            flipV += w * (g.fi1 - g.fi2);
            // picX += w * g.x1;
          }
        ...
        if (useAPIC) {
          ...
          // Finish computing FLIP velocity: v_p^n+1 = v_p^n + dt (v_i^n+1 -
          // v_i^n) * wip
          flipV = m_V[i] + (dt * flipV);
          m_V[i] = (flipPicRatio * flipV) +
                   ((1.0 - flipPicRatio) * picV); // blended velocity
        }

        // m_X[i] = picX; //use PIC for advection
        m_X[i] += m_V[i] * dt; // advect using velocity either from PIC (APIC or
                               // full PIC) or from FLIP (PIC blend FLIP)
      }
    });

    // TOP PLANE
    // Now use the field velocities to update the top and bottom crack planes
    // --> strategy for now is to apply all upward field velocities to top plane,
    // and all downward velocities to bottom plane (this is naive and only for
    // SENT)
    grid.parallel_for([&](int i) {
      if (i >= topPlane_startIdx &&
          i < bottomPlane_startIdx) { // only process the top plane particles
        Vector<T, dim> &Xp = m_X[i];
        Vector<T, dim> picV = Vector<T, dim>::Zero();
        Vector<T, dim> flipV = Vector<T, dim>::Zero();
        BSplineWeights<T, dim> spline(Xp, dx);
        ...
        grid.iterateKernel(spline, [&](const Vector<int, dim> &node, int oidx,
          ...
          // these steps depend on which field the particle is in
          if (g.separable != 1) {
            // treat as single field node -> if upward, add contribution for top
            // crack
            if (g.v1[1] > 0) {
              picV += w * g.v1;
              flipV += w * (g.v1 - g.vn1);
    ...
    grid.parallel_for([&](int i) {
      if (i >= bottomPlane_startIdx) { // only process the top plane particles
        ...
        grid.iterateKernel(spline, [&](const Vector<int, dim> &node, int oidx,
          ...
          // these steps depend on which field the particle is in
          if (g.separable != 1) {
            // treat as single field node -> if downard, add contribution for
            // bottom crack
            if (g.v1[1] < 0) {
              picV += w * g.v1;
              flipV += w * (g.v1 - g.vn1);
        ...
        // (left cracks and middle cracks ONLY)
...
/*Iterate particles to compute stress at various distances from the crack tip -
 * we want to plot sigma_yy vs r */
template <class T, int dim> class StressSnapshotOp : public AbstractOp {
public:
  using SparseMask = typename DFGMPM::DFGMPMGrid<T, dim>::SparseMask;
  Field<Vector<T, dim>> &m_X;
  ...
  void operator()() {
    ...
    grid.serial_for([&](int i) {
      if (!grid.crackInitialized ||
        ...
        // Only evaluate theta = 0, so within some y-trheshold from the central
        // crack AND only ahead of the crack
        T maxY = crackTip[1] + halfEnvelope;
        T minY = crackTip[1] - halfEnvelope;
        T minX = crackTip[0] + (2.0 * dx);
        if (pos[1] < maxY && pos[1] > minY && pos[0] > minX) {
          radius = std::sqrt((pos[0] - crackTip[0]) * (pos[0] - crackTip[0]) +
                             (pos[1] - crackTip[1]) * (pos[1] - crackTip[1]));
          sigmaYY = sigma(1, 1);
...
template <class T, int dim> class SimpleDampingOp : public AbstractOp {
public:
  using SparseMask = typename DFGMPM::DFGMPMGrid<T, dim>::SparseMask;
  Field<Vector<T, dim>> &m_V;
...
/* Compute the J Integral using a rectangular path of grid nodes centered on the
 * closest node to the crack tip */
template <class T, int dim> class ComputeJIntegralOp : public AbstractOp {
public:
  using SparseMask = typename DFGMPM::DFGMPMGrid<T, dim>::SparseMask;
  Field<Vector<T, dim>> &m_X;
  ...
  void operator()(int contourRadius, std::ofstream &file) {
    ...
    // NOTE: This routine is designed for HORIZONTAL LEFT SIDE CRACKS (b.c. of
    // contour intersection assumptions --> counter clockwise path hits top of
    // crack first)
    ...
    // --> COUNTER CLOCKWISE STARTING FROM TOP LEFT NODE
    ...
    grid.iterateRectangularContour(
        spline, contourRadius,
        [&](const Vector<int, dim> &node, DFGMPM::GridState<T, dim> &g) {
          Vector<T, dim> xi = node.template cast<T>() * dx;
          // Grab data
    ...
    //     file << "idx: " << i <<  ", xi: (" << contourPoints[i][0] << "," <<
    //     contourPoints[i][1] << "), separable:" <<
    //     contourGridStates[i]->separable << "\n";
    // }

    // STEP 1: Construct ordered list of contour points starting with
    // intersection with bottom of crack, counter-clockwise around contour, and
    // then ending with intersection with top of crack
    bool foundBottomIntersection = false;
    bool foundTopIntersection = false;
    // store the first point index for the segment intersecting with bottom and
    // top crack planes
    ...
    // STEP 1b: Now look for the lower intersection -> start with
    // topIntersectionIdx since this is the first segment that can have an
    // intersection with the top points
    ...
    T epsilon = 1e-9;
    if (std::abs(bottomIntersection[1] -
                 contourPoints[bottomIntersectionIdx + 1][1]) < epsilon) {
      bottomIntersectionIdx++;
    }

    // STEP 1c: Check that crack is actually open (if it is not, we cannot
    // compute J integral)
    ...
    if (std::abs(topIntersection[1] - bottomIntersection[1]) < epsilon) {
      std::cout << "Crack is not opened at the intersection points, cannot "
                   "compute J-integral!"
                << std::endl;

      // We construct all of the pieces we need to finally compute the J
      // integral noting that the crack is closed here Specifically, a list of
      // contour points starting with crack intersection and going
      // counter-clockwise around back to the crack intersection

      // Crack intersection
      finalContourPoints.push_back(bottomIntersection);
      finalContourGridStates.push_back(
          contourGridStates[bottomIntersectionIdx]); // store the grid state for
                                                     // the START point of the
                                                     // contour segment that
                                                     // intersected the crack

      // now add points counter-clockwise until hit end of list (then we will
      // start from beginning until bottom intersecting segment)
      for (int i = bottomIntersectionIdx + 1; i < (int)contourPoints.size();
           ++i) { // end of list
      ...
      finalContourGridStates.push_back(
          contourGridStates[topIntersectionIdx]); // store grid state from START
                                                  // of top intersecting segment
    } else {
      // Else we construct all of the pieces we need to finally compute the J
      // integral! Specifically, a list of contour points starting with bottom
      // intersection and going counter-clockwise around to the top intersection
    ...
    std::vector<T> Fm_I_SegmentList; // Store Fm1 and Fm2 for each line segment
    ...
    std::vector<Matrix<T, dim, dim>>
        m_Fi_Interpolated; // collect the reconstructed Fi's that were
                           // interpolated between for x_bottom and x_top

    T J_I = 0;  // set J integral mode I to 0 for now
    T J_II = 0; // set J integral mode II to 0 for now
    for (int i = 0; i < (int)finalContourPoints.size() - 1;
         ++i) {     // iterate contour segments
      T Fsum_I = 0; // this is what we focus on setting for each segment (three
                    // cases below)
      T Fsum_II = 0; // mode II
      ...
      if (i == 0) { // first segment
        // If first segment, first we compute interpolations for bottom
        // intersection point
        ...
        // Interpolate the deformation gradient ingredients! --> USE FIELD 1 FOR
        // BOTTOM INTERSECT!!!
        DFGMPM::GridState<T, dim> *gi1 = contourGridStates
            [bottomIntersectionIdx]; // NOTE: this will very likely be a
                                     // separable node!!!! only use field 1 for
                                     // interpolating bottom intersect
        DFGMPM::GridState<T, dim> *gi2 =
            contourGridStates[bottomIntersectionIdx +
                              1]; // grab the two grid states to interpolate
                                  // between
        ...
        // sigmaBlend = (gi1->sigma1 * (1 - blendRatio)) +  (gi2->sigma1 *
        // blendRatio); UquatBlend = (gi1->Uquat1 * (1 - blendRatio)) +
        // (gi2->Uquat1 * blendRatio); VquatBlend = (gi1->Vquat1 * (1 -
        // blendRatio)) +  (gi2->Vquat1 * blendRatio); //NOTE WE USE FIELD 1
        // VALUES FOR BOTTOM INTERSECT!! Fi1 = computeF(sigmaBlend, UquatBlend,
        // VquatBlend);
        Fi1 = ((gi1->Fi1 * gi1->m1 * (1 - blendRatio)) +
               (gi2->Fi1 * gi2->m1 * blendRatio)) /
              (gi1->m1 + gi2->m1); // mass weighted 1-D interpolation
        m_Fi.push_back(Fi1);
        ...
        // Interpolate the deformation gradient ingredients! --> USE FIELD 2 FOR
        // TOP INTERSECT!!!
        DFGMPM::GridState<T, dim> *gi1 = contourGridStates[topIntersectionIdx];
        DFGMPM::GridState<T, dim> *gi2 = contourGridStates
            [topIntersectionIdx +
             1]; // grab the two grid states to interpolate between -> this one
                 // is very likely separable!! use field 2 only for interpolating
        // Finterp1 = computeF(gi1->sigma1, gi1->Uquat1, gi1->Vquat1);
        Finterp1 = gi1->Fi1;
        m_Fi_Interpolated.push_back(Finterp1);
        // Vector<T, dim> sigmaBlend;
        ...
        if (gi2->separable == 1) {
          ...
          // sigmaBlend = (gi1->sigma1 * (1 - blendRatio)) +  (gi2->sigma2 *
          // blendRatio); UquatBlend = (gi1->Uquat1 * (1 - blendRatio)) +
          // (gi2->Uquat2 * blendRatio); VquatBlend = (gi1->Vquat1 * (1 -
          // blendRatio)) +  (gi2->Vquat2 * blendRatio); Finterp2 =
          // computeF(gi2->sigma2, gi2->Uquat2, gi2->Vquat2);
          Finterp2 = gi2->Fi2; // NOTE WE USE FIELD 2 VALUES FOR TOP INTERSECT!!
          Fi2 = ((gi1->Fi1 * gi1->m1 * (1 - blendRatio)) +
                 (gi2->Fi2 * gi2->m2 * blendRatio)) /
                (gi1->m1 +
                 gi2->m2); // mass weighted 1-D interpolation, using field 2
        } else {
          file << "Top Intersect Interpolated Using Field 1\n";
          // sigmaBlend = (gi1->sigma1 * (1 - blendRatio)) +  (gi2->sigma1 *
          // blendRatio); UquatBlend = (gi1->Uquat1 * (1 - blendRatio)) +
          // (gi2->Uquat1 * blendRatio); VquatBlend = (gi1->Vquat1 * (1 -
          // blendRatio)) +  (gi2->Vquat1 * blendRatio); Finterp2 =
          // computeF(gi2->sigma1, gi2->Uquat1, gi2->Vquat1);
          Finterp2 = gi2->Fi1; // if not separable, just use field 1
          Fi2 = ((gi1->Fi1 * gi1->m1 * (1 - blendRatio)) +
                 (gi2->Fi1 * gi2->m1 * blendRatio)) /
                (gi1->m1 +
                 gi2->m1); // mass weighted 1-D interpolation, using field 1
        }
        ...
        // Compute F for each endpoint --> NEITHER should be separable in this
        // case
        // Fi1 = computeF(g1->sigma1, g1->Uquat1, g1->Vquat1);
        // Fi2 = computeF(g2->sigma1, g2->Uquat1, g2->Vquat1); //note we only
        // access field 1 for both
        Fi1 = g1->Fi1;
        Fi2 = g2->Fi1;
        m_Fi.push_back(Fi1);
        m_Fi.push_back(Fi2);
      ...
      // Now after computing Fsum using one of three cases, we can add this
      // contribution to the Jintegral!
      T deltaI =
          std::sqrt((x1[0] - x2[0]) * (x1[0] - x2[0]) +
                    (x1[1] - x2[1]) *
                        (x1[1] - x2[1])); // compute distance from x1 to x2 (the
                                          // end points of current segment)
    ...
    file << "====================================================== J-Integral "
            "Computation using "
         << contourRadius << "x" << contourRadius
         << " Contour ======================\n";
    file << "Bottom Intersection | Idx: " << bottomIntersectionIdx
         << " Point: (" << bottomIntersection[0] << "," << bottomIntersection[1]
         << "), Blend Ratio: " << blendRatios[0] << " \n";
    file << "Top Intersection | Idx: " << topIntersectionIdx << " Point: ("
         << topIntersection[0] << "," << topIntersection[1]
         << "), Blend Ratio: " << blendRatios[1] << "\n";
    // for(int i = 0; i < (int)finalContourPoints.size(); ++i){
    //     file << "idx:" << i << ", Point: (" << finalContourPoints[i][0] <<
    //     "," << finalContourPoints[i][1] << "), Fsum_I: " << Fsum_I_List[i] <<
    //     " \n";
    // }
    file << "------Bottom Intersection Interpolation (first point in "
            "contour)------\n";
    file << "Finterp1:" << m_Fi_Interpolated[0] << "\n";
    file << "Finterp2:" << m_Fi_Interpolated[1] << "\n";
    for (int i = 0; i < (int)finalContourPoints.size() - 1; ++i) {
      file << "-----------------<Line Segment " << i
           << ", Fsum_I: " << Fsum_I_List[i] << ", Delta_I: " << DeltaI_List[i]
           << ", J_I Contribution: " << Fsum_I_List[i] * (DeltaI_List[i] / 2.0)
           << ">-----------------\n";
      file << "idx1: " << i << ", Point: (" << finalContourPoints[i][0] << ","
           << finalContourPoints[i][1] << "), Fm_I: " << Fm_I_SegmentList[i * 2]
           << ", Normal: [" << Fm_I_NormalX[i * 2] << "," << Fm_I_NormalY[i * 2]
           << "], W: " << Fm_I_W[i * 2] << ", termTwo: " << Fm_I_termTwo[i * 2]
           << "\nFi1: " << m_Fi[i * 2] << " \n";
      file << "idx2: " << i + 1 << ", Point: (" << finalContourPoints[i + 1][0]
           << "," << finalContourPoints[i + 1][1]
           << "), Fm_I: " << Fm_I_SegmentList[(i * 2) + 1] << ", Normal: ["
           << Fm_I_NormalX[(i * 2) + 1] << "," << Fm_I_NormalY[(i * 2) + 1]
           << "], W: " << Fm_I_W[(i * 2) + 1]
           << ", termTwo: " << Fm_I_termTwo[(i * 2) + 1]
           << "\nFi2: " << m_Fi[(i * 2) + 1] << " \n";
    }
    file << "------Top Intersection Interpolation (last point in "
            "contour)------\n";
    ...
    // }
    // for(int i = 0; i < (int)finalContourPoints.size(); ++i){
    //     file << "idx " << i << " position: (" << finalContourPoints[i][0] <<
    //     "," << finalContourPoints[i][1] << ")\n";
    // }
    // for(int i = 0; i < (int)contourPoints.size(); ++i){
    //     file << "idx " << i << " pointer: " << contourGridStates[i] << "\n";
    // }
    // for(int i = 0; i < (int)contourPoints.size(); ++i){
    //     file << "idx " << i << " position: (" << contourPoints[i][0] << ","
    //     << contourPoints[i][1] << ")\n";
    // }
    file << "J_I: " << J_I << "\n";
    file << "J_II: " << J_II << "\n";
  ...
  // Compute F_i based on singular values and quaternion rotations from F = U *
  // Sigma * V^T
  Matrix<T, dim, dim> computeF(Vector<T, dim> singularValues,
                               Vector<T, 4> Uquat, Vector<T, 4> Vquat) {
  ...
  // Compute Fm based on grid data at node i, the line segment between the
  // nodes, and the mode (0 for x, 1 for y)
  std::vector<T> computeFm(Matrix<T, dim, dim> Fi, Vector<T, dim> lineSegment,
                           int mode) {
    ...
    // intersect = 0 for regular call to Fm (we don't expect this to ever be a
    // separable node) intersect = 1 for a bottom intersect interpolation with
    // the separable node -> use only Field 1 intersect = 2 for a top intersect
    // interpolation with the separable node -> use only Field 2 NOTE: for
    // separable nodes we will ONLY use one or the other field, the field that
    // points in the direction of the current segment NOTE: so, for example, we
    // use field 1 (points down from crack) for interpolating the bottom
    // intersect, and field 2 (points up from crack) for the top intersect Now we
    // must reconstruct our deformation gradient from the singular values and
    // quaternion rotations F = U Sigma V^T

    // Compute strain energy density, W (elastic potential energy density)
    // NOTE: It is MUCH easier to hardcode the elasticity model here, so if we
    // change the constitutive model we NEED to change this here as well!!
    T W = 0;
    T termTwo = 0;
    int elasticityMode = 1; // 0 = LINEAR, 1 = FCR
    if (elasticityMode == 0) {
      // LINEAR ELASTICITY
      Matrix<T, dim, dim> epsilon =
          0.5 * (Fi + Fi.transpose()) - Matrix<T, dim, dim>::Identity();
      T tr_epsilon = epsilon.diagonal().sum();
      W = mu * epsilon.squaredNorm() +
          la * 0.5 * tr_epsilon * tr_epsilon; // W = psi(epsilon)
      ...
      W = mu * (sigma - Vector<T, dim>::Ones()).squaredNorm() +
          T(0.5) * la * std::pow(sigma.prod() - T(1), 2);
  ...
  // Elegant line segment intersection check from
  ...
  bool intersect(Vector<T, dim> &A, Vector<T, dim> &B, Vector<T, dim> &C,
                 Vector<T, dim> &D) {
    return ccw(A, C, D) != ccw(B, C, D) && ccw(A, B, C) != ccw(A, B, D);
  }

  // Function to compute the intersection point (we'll only use this if we know
  // there is an intersection) - from here:
  // https://stackoverflow.com/questions/563198/how-do-you-detect-where-two-line-segments-intersect
  void getIntersection(Vector<T, dim> &A, Vector<T, dim> &B, Vector<T, dim> &C,
                       Vector<T, dim> &D, Vector<T, dim> &intersection) {
    T s1x = B[0] - A[0];
    T s1y = B[1] - A[1];
    T s2x = D[0] - C[0];
    T s2y = D[1] - C[1];
    T t =
        (s2x * (A[1] - C[1]) - s2y * (A[0] - C[0])) / (-s2x * s1y + s1x * s2y);
    intersection[0] = A[0] + (t * s1x);
    intersection[1] = A[1] + (t * s1y);
  }
};

/* Transfer particle stress, nominal stress, and def grad to grid so we can
 * compute J Integral*/
template <class T, int dim>
  ...
  void operator()() {
    ...
    grid.colored_for([&](int i) {
      if (!grid.crackInitialized ||
        ...
        // We will transfer the deformation gradient through transferring its
        // singular values and rotations (as quaternions) -> three separate
        // intrinsic transfers here: sigma, Uquat, Vquat
        Matrix<T, dim, dim> U, V;
        Vector<T, dim> sigma;
        ...
        grid.iterateKernel(spline, [&](const Vector<int, dim> &node, int oidx,
          ...
          // We will intrinsically transfer here the singular values and
          // quaternions for U and V rotations from F = UsigmaV^T Storing
          // accumulated weights in gridSeparability[0] (field 1 weight sum) and
          // gridSeparability[1] (field 2 weight sum)
          ...
          if (g.separable != 1 || !useDFG) {
            ...
            if (fieldIdx == 0) {
              ...
              // g.fi2 += stress * dw; //transfer stress to grid, fi (second
              // field)
              g.sigma2 += sigma * w;
              g.Uquat2 += Uquat * w;
              g.Vquat2 += Vquat * w;
              g.gridSeparability[1] +=
                  w; // sum up the total weight (second field)
            }
          }
        });
      }
    ...
    /* Iterate grid to divide out the total weight sums for P and F since we
     * transfer these intrinsically */
    grid.iterateGrid(
        [&](const Vector<int, dim> &node, DFGMPM::GridState<T, dim> &g) {
