...
template <class T, int dim> class BackGridSortOp : public AbstractOp {
public:
...
template <class T, int dim> class NeighborSortOp : public AbstractOp {
public:
  using SparseMask = typename DFGMPMGrid<T, dim>::SparseMask;
  Field<Vector<T, dim>> &m_X;
  ...
  void operator()() {
    ...
    grid.parallel_for([&](int p_i) {
      if (!grid.crackInitialized ||
          p_i < grid.crackParticlesStartIdx) { // skip crack particles if we
                                               // have them
        const Vector<T, dim> pos_i = m_X[p_i];
        BSplineWeights<T, dim> spline(pos_i, rp);
        particleNeighbors[p_i].clear(); // empty neighbor list for this particle
                                        // before we fill it up
        grid.iterateNeighbors(
            spline, [&](const Vector<int, dim> &node, GridState<T, dim> &g) {
              // Iterate through all particles mapped to this node to see if
              // they neighbor the current particle
              for (size_t j = 0; j < g.mappedParticles.size(); ++j) {
                ...
                if ((p_i != p_j) && (dist < rp)) {
                  // if particle is not the same particle AND distance is less
                  // than kernel radius, add to neighbor list
...
/* Surface Detection (set sp values before DGs!!!)
   NOTE: Set whether this particle is a surface particle or not by comparing the
   kernel sum, S(x), against a user threshold (st) */
template <class T, int dim> class SurfaceDetectionOp : public AbstractOp {
public:
  using SparseMask = typename DFGMPMGrid<T, dim>::SparseMask;
  Field<Vector<T, dim>> &m_X;
  ...
  void operator()() {
    ...
    tbb::parallel_for(
        tbb::blocked_range<int>(0, particleNeighbors.size()),
        [&](tbb::blocked_range<int> b) {
          for (int p_i = b.begin(); p_i < b.end(); ++p_i) {
            if (!grid.crackInitialized ||
                p_i < grid.crackParticlesStartIdx) { // skip crack particles if
                                                     // we have them
              const Vector<T, dim> pos_i = m_X[p_i];
              T S = 0.0;
              for (size_t j = 0; j < particleNeighbors[p_i].size();
                   ++j) { // iter particle neighbors
...
/* Compute DG for all particles and for all nodes, because of colored looper we
 * can do this more easily! */
template <class T, int dim> class ComputeDamageGradientsOp : public AbstractOp {
public:
  using SparseMask = typename DFGMPMGrid<T, dim>::SparseMask;
  Field<Vector<T, dim>> &m_X;
  Bow::Field<std::vector<int>> &particleNeighbors; // neighbor list
  ...
  void operator()() {
    ...
    grid.colored_for([&](int p_i) {
      if (!grid.crackInitialized ||
        ...
        // Now, we still have to iterate over all grid nodes that this particle
        // maps to so we can compute the gridDG as the maximum norm particle DG
        // mapping to it! NOTE: we use colored looper in this op to prevent race
        // conditions
        BSplineWeights<T, dim> spline(pos_i, dx);
        grid.iterateKernelWithoutWeights(
            spline, [&](const Vector<int, dim> &node, GridState<T, dim> &g) {
...
/* Partition particles into field 1 or field 2 while transferring mass to the
proper field NOTE: mass transfer occurs here for DFG (but velocity transfer is
later after computing separability) */
template <class T, int dim> class PartitioningOp : public AbstractOp {
public:
  using SparseMask = typename DFGMPMGrid<T, dim>::SparseMask;
  Field<Vector<T, dim>> &m_X;
  std::vector<T> &m_mass;
  ...
  void operator()() {
    ...
    grid.colored_for([&](int i) {
      if (!grid.crackInitialized ||
        ...
        particleAF[i]
            .clear(); // clear out this particle's activeField list because we
                      // are about to fill it for this substep

        T maxD = std::max(Dp[i],
                          (T)sp[i]); // use max of damage and surface particle
                                     // markers so we detect green case correctly
        ...
        grid.iterateKernel(spline, [&](const Vector<int, dim> &node, int oidx,
          ...
          if (particleDG[i].dot(g.gridDG) >= 0) {
            ...
            particleAF[i].push_back(
                0); // set this particle's AF to 0 for this grid node
            if (g.gridMaxDamage[0] < maxD) {
              g.gridMaxDamage[0] = maxD; // compute the max damage seen in this
                                         // field at this grid node
            }
          } else {
            // field 2
            g.m2 += mass * w;
            ...
            particleAF[i].push_back(
                1); // set this particle's AF to 1 for this grid node
            if (g.gridMaxDamage[1] < maxD) {
              g.gridMaxDamage[1] = maxD; // compute the max damage seen in this
                                         // field at this grid node
            }
          }
        });
    ...
    grid.iterateGrid([&](const Vector<int, dim> &node, GridState<T, dim> &g) {
      ...
      if (g.gridSeparability[2] > 0) {
        g.gridSeparability[0] /=
            g.gridSeparability[2]; // divide numerator by denominator
      } else {
      ...
      if (g.gridSeparability[3] > 0) {
        g.gridSeparability[1] /=
            g.gridSeparability[3]; // divide numerator by denominator
      } else {
        g.gridSeparability[1] = 0.0;
      }

      // NOTE: separable = 0 indicates single field node, separable = 1
      // indicates two field node
      if (g.m1 > 0 && g.m2 > 0) {
        T minSep, maxMax;
        ...
        if (maxMax >= minDp && minSep > dMin) {
          ...
          // in this case, treat this node as a single field, so we accumulate
          // the mass in m1
          g.separable = 0;
          g.m1 += g.m2;
          g.m2 = 0.0;
...
/* Transfer damage to the grid, then compute for each particle the damage
 * Laplacian */
...
public:
  ...
  void operator()() {
    ...
    // P2G STYLE COLORED PARALLEL LOOP: here we transfer numerator and
    // denominator for grid damage
    grid.colored_for([&](int i) {
      if (!grid.crackInitialized ||
          i < grid.crackParticlesStartIdx) { // skip crack particles if we have
                                             // them
        ...
        grid.iterateKernel(spline, [&](const Vector<int, dim> &node, int oidx,
          ...
          if (g.separable != 1) {
            ...
            // two-field treatment -- must transfer to the correct field in the
            // separable grid nodes
            int fieldIdx =
    ...
    // PARALLEL GRID ITERATION: now we divide out the denominator to get final
    // d_i
    grid.iterateGrid([&](const Vector<int, dim> &node, GridState<T, dim> &g) {
    ...
    // G2P STYLE PARALLEL PARTICLE LOOP: now we compute each particle's damage
    // laplacian
    grid.parallel_for([&](int i) {
      if (!grid.crackInitialized ||
        ...
        grid.iterateKernelWithLaplacian(
            spline, [&](const Vector<int, dim> &node, int oidx, T w,
                        Vector<T, dim> dw, T laplacian, GridState<T, dim> &g) {
              if (g.separable != 1) {
                // single field treatment
                m_damageLaplacians[i] += g.d1[0] * laplacian;
              } else {
                // two-field treatment -- must transfer to the correct field in
                // the separable grid nodes
                int fieldIdx =
                    particleAF[i][oidx]; // grab the field that this particle
                                         // belongs in for this grid node (oidx)
                if (fieldIdx == 0) {
...
template <class T, int dim> class UpdateAnisoMPMDamageOp : public AbstractOp {
public:
  using SparseMask = typename DFGMPMGrid<T, dim>::SparseMask;
  std::vector<T> &m_Dp;               // particle damage
  std::vector<T> &m_damageLaplacians; // particle damage Laplacians
    ...
    grid.parallel_for([&](int i) {
      if (!grid.crackInitialized ||
        ...
        for (int i = 0; i < dim; i++) {
          ...
          sigmaPlus += ((lambda + std::abs(lambda)) / 2.0) *
                       (eigenVecs * eigenVecs.transpose());
        }

        // Structural Tensor (dictates anisotropy)
        Matrix<T, dim, dim> A = Matrix<T, dim, dim>::
            Identity(); // TODO: insert real structural tensor eventually, but
                        // for now this will be hardcoded to be isotropic
        ...
        T dTilde = std::max(
            m_dTildeH[i],
            zeta * (((phi - 1) + std::abs(phi - 1)) /
                    2.0));     // remember macaulay function is y(x) = (x+|x|)/2
        ...
        T newD = Dp + ((dt / eta) *
                       ((diff + std::abs(diff)) /
                        2.0)); // macaulay ensures that we only update when we
...
public:
  ...
  void operator()() {
    ...
    grid.iterateGrid([&](const Vector<int, dim> &node, GridState<T, dim> &g) {
      ...
      if (symplectic) {
        gravity_term[1] =
            gravity * dt; // only nonzero if symplectic, else we add gravity in
                          // residual of implicit solve
      }
      T mass1 = g.m1;
      Vector<T, dim> alpha1;
...
/* Iterate grid to calculate impulse and either apply it (symplectic) or save
 * for later (implicit) */
template <class T, int dim> class ApplyImpulseOp : public AbstractOp {
public:
  using SparseMask = typename DFGMPMGrid<T, dim>::SparseMask;
  Vector<T, dim> center;
  T strength;
  ...
  void operator()() {
    ...
    /* Iterate grid to calculate impulse and either apply it (symplectic) or
     * save for later (implicit) */
    grid.iterateGrid([&](const Vector<int, dim> &node, GridState<T, dim> &g) {
...
NOTE: We will directly apply these, but it happens at different stages in the
pipeline for explicit/implicit */
template <class T, int dim> class ContactForcesOp : public AbstractOp {
public:
  using SparseMask = typename DFGMPMGrid<T, dim>::SparseMask;
  ...
  void operator()() {
    ...
    grid.iterateGrid([&](const Vector<int, dim> &node, GridState<T, dim> &g) {
      ...
      if (g.separable == 1) {
        ...
        n_1.normalize(); // in place normalize (NOTE: also makes sure no div by
                         // zero)
        n_2.normalize();
        ...
        if (symplectic ||
            !useImplicitContact) { // only compute these forces for explicitMPM
                                   // or if we want to use explicitContact in
                                   // implicitMPM
          ...
          if constexpr (dim == 2) { // compute tangent component for 2D
            ...
            if (v1x == 0 && v1y == 0 && v1z != 0) {
              ...
              // (x x x), (0 x x), (x 0 x)
              // Let v2x = v1x, v2y = 2*v1y to prevent linear dependence on v1.
              // Then set v3 based on solving v1 dot v2 = 0
              T v2x = 1 * v1x;
              T v2y = 2 * v1y;
          ...
          T fTanSign1 =
              (fTanMag1 > 0) ? 1.0 : 0.0; // NOTE: L2 norm is always >= 0
          T fTanSign2 = (fTanMag2 > 0) ? 1.0 : 0.0;

          // Tangent directions, all 0 if sign of mag was 0
          Vector<T, dim> tanDirection1 = (fTanSign1 == 0)
                                             ? Bow::Vector<T, dim>::Zero()
                                             : fTanComp1.normalized();
          Vector<T, dim> tanDirection2 = (fTanSign2 == 0)
                                             ? Bow::Vector<T, dim>::Zero()
                                             : fTanComp2.normalized();

          // Finally compute contact forces!
          // NOTE: we ONLY compute this if we detect interpenetration b/w the
          // fields
          ...
          if ((v_cm - v_1).dot(n_cm1) + (v_cm - v_2).dot(n_cm2) < 0) {
            ...
            T tanMin1 = (fricCoeff * std::abs(fNormal1) < std::abs(fTanMag1))
                            ? (fricCoeff * std::abs(fNormal1))
                            : std::abs(fTanMag1);
            T tanMin2 = (fricCoeff * std::abs(fNormal2) < std::abs(fTanMag2))
                            ? (fricCoeff * std::abs(fNormal2))
                            : std::abs(fTanMag2);
  ...
  void operator()() {
    ...
    grid.iterateGrid([&](const Vector<int, dim> &node, GridState<T, dim> &g) {
      ...
      if (g.separable == 1) {
        ...
        g.v2 = new_v2;
        g.x2 = node.template cast<T>() * dx +
               dt * new_v2; // advect second field nodal position separately...
                            // this feels weird
      }
    });
  }
...
public:
  ...
  void operator()() {
    ...
    grid.iterateGrid([&](const Vector<int, dim> &node, GridState<T, dim> &g) {
      BC.mpm_implicit_update(node.template cast<T>() * dx, BC_basis[g.idx],
                             BC_order[g.idx]);
    });
    if (sdof > 0) {
      grid.iterateGrid([&](const Vector<int, dim> &node, GridState<T, dim> &g) {
        BC.mpm_implicit_update(node.template cast<T>() * dx,
                               BC_basis[ndof + g.sep_idx],
                               BC_order[ndof + g.sep_idx]);
      });
    }
  }
};

template <class T, int dim> class GridToParticlesOp : public AbstractOp {
public:
  Field<Vector<T, dim>> &m_X;
  Field<Vector<T, dim>> &m_V;
  Field<Matrix<T, dim, dim>> &m_C;
  ...
  template <bool useAPIC> void gridToParticles() {
    BOW_TIMER_FLAG("G2P");
    ...
    grid.parallel_for([&](int i) {
      if (!grid.crackInitialized ||
        ...
        grid.iterateKernel(spline, [&](const Vector<int, dim> &node, int oidx,
          ...
          Vector<T, dim> xn =
              node.template cast<T>() * dx; // same regardless of separability
        ...
        if constexpr (useAPIC) {
          ...
          grid.iterateKernel(spline, [&](const Vector<int, dim> &node, int oidx,
            ...
            if (g.separable != 1 || !useDFG) {
              Bp += 0.5 * w *
                    (g_v_new * (xn - m_X[i] + g.x1 - picX).transpose() +
                     (xn - m_X[i] - g.x1 + picX) * g_v_new.transpose());
            } else if (g.separable == 1) {
              ...
              if (fieldIdx == 0) {
                ...
                Bp += 0.5 * w *
                      (g_v2_new * (xn - m_X[i] + g.x2 - picX).transpose() +
                       (xn - m_X[i] - g.x2 + picX) * g_v2_new.transpose());
              }
            }
          });
          m_C[i] = Bp * D_inverse;
...
public:
  ...
  void operator()() {
    ...
    // grid.countNumNodes();
    grid.iterateGridSerial(
        [&](const Vector<int, dim> &node, GridState<T, dim> &g) {
